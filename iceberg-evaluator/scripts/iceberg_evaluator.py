...
BENCHMARK_OVERCLAIM_PATTERNS = (
    r"(?:完全|高度|显著)?(?:匹配|满足|达到|符合|高于|低于|优于|超过|超出|略超).{0,12}(?:基准|画像|必须项)",
    r"(?:基准|画像|必须项).{0,12}(?:完全|高度|显著)?(?:匹配|满足|达到|符合|高于|低于|优于|超过|超出|略超)",
)
...
FOLLOWUP_SCHEMA = {
    ...
    "required": ["question", "purpose", "expected_evidence", "source_question_id"],
    "additionalProperties": False,
}
...
def validate_benchmark(data: dict[str, Any]) -> None:
    ...
    require(
        success_definition,
        ["observation_months", "retention_definition", "internalization_definition", "growth_definition"],
        "benchmark.success_definition",
    )
    months = success_definition.get("observation_months")
    if isinstance(months, bool) or not isinstance(months, (int, float)) or months <= 0:
        raise ValueError("benchmark.success_definition.observation_months 必须是正数。")
    for field in ("retention_definition", "internalization_definition", "growth_definition"):
        if not isinstance(success_definition.get(field), str) or not success_definition[field].strip():
            raise ValueError(f"benchmark.success_definition.{field} 必须为非空字符串。")
    ...
    for item in indicators:
        ...
        observable = item.get("observable_evidence")
        if not isinstance(observable, list) or not observable or not all(isinstance(x, str) and x.strip() for x in observable):
            raise ValueError(f"above_water_indicators.{indicator}.observable_evidence 必须是非空字符串数组。")
    ...
    for dim in DIMENSIONS:
        ...
        if not isinstance(detail.get("definition"), str) or not detail["definition"].strip():
            raise ValueError(f"benchmark.dimensions.{dim}.definition 必须为非空字符串。")
        anchors = detail.get("behavioral_anchors")
        if not isinstance(anchors, list) or not anchors or not all(isinstance(x, str) and x.strip() for x in anchors):
            raise ValueError(f"benchmark.dimensions.{dim}.behavioral_anchors 必须是非空字符串数组。")
    ...
    if data["benchmark_status"] == "confirmed":
        governance = data.get("governance")
        if not isinstance(governance, dict):
            raise ValueError("confirmed 画像必须提供 benchmark.governance 业务确认记录。")
        require(
            governance,
            ["business_owner", "hr_owner", "approved_at", "sample_size", "outcome_data_source", "approval_note"],
            "benchmark.governance",
        )
        for field in ("business_owner", "hr_owner", "approved_at", "outcome_data_source", "approval_note"):
            if not isinstance(governance.get(field), str) or not governance[field].strip():
                raise ValueError(f"benchmark.governance.{field} 必须为非空字符串。")
        sample_size = governance.get("sample_size")
        if isinstance(sample_size, bool) or not isinstance(sample_size, int) or sample_size <= 0:
            raise ValueError("benchmark.governance.sample_size 必须是正整数。")
...
def resolve_model_config(model: str | None = None, base_url: str | None = None, output_mode: str | None = None) -> dict[str, Any]:
    ...
    api_key = dashscope_key if dashscope_detected else openai_key
...
def _require_string_list(value: Any, label: str) -> list[str]:
    if not isinstance(value, list) or not all(isinstance(item, str) for item in value):
        raise ValueError(f"模型输出 {label} 必须是字符串数组。")
...
def normalize_llm_output(llm: Any) -> dict[str, Any]:
    ...
    if missing:
        raise ValueError(f"模型输出缺少必填字段：{', '.join(missing)}。")
    if not isinstance(llm["above_water_summary"], list):
        raise ValueError("模型输出 above_water_summary 必须是数组。")
    if not all(isinstance(item, dict) for item in llm["above_water_summary"]):
        raise ValueError("模型输出 above_water_summary 每项必须是对象。")
    llm["key_strengths"] = _require_string_list(llm["key_strengths"], "key_strengths")
    llm["risk_flags"] = _require_string_list(llm["risk_flags"], "risk_flags")
    llm["analysis_limitations"] = _require_string_list(llm["analysis_limitations"], "analysis_limitations")
    if not isinstance(llm["llm_report"], str):
        raise ValueError("模型输出 llm_report 必须是字符串。")
    ...
    for dim in DIMENSIONS:
        ...
        dim_required = (
            "score", "signals", "evidence", "counter_evidence", "evidence_coverage",
            "confidence", "benchmark_gap", "validation_questions",
        )
        dim_missing = [key for key in dim_required if key not in detail]
        if dim_missing:
            raise ValueError(f"维度 {dim} 缺少字段：{', '.join(dim_missing)}。")
        detail["signals"] = _require_string_list(detail["signals"], f"iceberg_scores.{dim}.signals")
        for field in ("evidence", "counter_evidence", "validation_questions"):
            if not isinstance(detail[field], list):
                raise ValueError(f"模型输出 iceberg_scores.{dim}.{field} 必须是数组。")
        if not isinstance(detail["benchmark_gap"], str):
            raise ValueError(f"模型输出 iceberg_scores.{dim}.benchmark_gap 必须是字符串。")
        try:
            detail["score"] = max(0, min(100, int(detail["score"])))
        except (TypeError, ValueError):
            raise ValueError(f"模型输出 iceberg_scores.{dim}.score 必须是 0-100 的整数。")
        try:
            detail["evidence_coverage"] = max(0, min(100, int(detail["evidence_coverage"])))
        except (TypeError, ValueError):
            raise ValueError(f"模型输出 iceberg_scores.{dim}.evidence_coverage 必须是 0-100 的整数。")
    return llm
...
def has_benchmark_overclaim(text: Any) -> bool:
    value = str(text or "")
    value = re.sub(
        r"(?:暂不|不能|无法|不足以|不应).{0,12}(?:判断|说明|证明).{0,12}(?:达到|符合|高于|低于|优于|超过|匹配).{0,8}(?:基准|画像)",
        "",
        value,
    )
    return any(re.search(pattern, value) for pattern in BENCHMARK_OVERCLAIM_PATTERNS)


def sanitize_conclusion_fields(llm: dict[str, Any]) -> bool:
    """移除模型自由文本中的确定性画像结论；基准差异只允许由规则字段表达。"""
    changed = False
    if has_benchmark_overclaim(llm.get("llm_report", "")):
        llm["llm_report"] = (
            "当前材料已提取冰山上事实与四维工作相关信号。请结合逐项证据、反向证据、"
            "覆盖率和置信度进行人工复核；画像差异仅按各维度的确定性比较状态解释。"
        )
        changed = True
    for field in ("key_strengths", "risk_flags"):
        original = list(llm.get(field, []))
        filtered = [item for item in original if not has_benchmark_overclaim(item)]
...
def post_process_dimension(
    ...
    detail["display_level"] = benchmark_status == "confirmed" and confidence in ("medium", "high")
...
def post_process(
    ...
    conclusion_sanitized = sanitize_conclusion_fields(llm)
    issues: list[dict[str, str]] = []
    ...
    if conclusion_sanitized:
        limitations.append("综合摘要或要点中含确定性画像判断，已移除并改为基于证据与置信度的中性表述。")
    if coverage < 55:
...
def check_decision_boundary(result: dict[str, Any]) -> list[str]:
    ...
    for name, text in fields:
        ...
        if has_benchmark_overclaim(text):
            violations.append(f"{name} 含未经规则生成的确定性画像判断")
    return violations
...
def dimension_card(key: str, detail: dict[str, Any], show_comparison: bool) -> str:
    ...
    display_level = bool(detail.get("display_level"))
    score_class = "dim-score" if display_level else "dim-score explore-score"
    score_caption = "" if display_level else "<small style='display:block;font-size:11px;font-weight:500'>探索性支持度</small>"
    level_tag = f"<span class='ctag ctag-level'>等级：{html.escape(level_label)}</span>" if display_level else ""
...
<div class='card-head'><span class='dim-name'>{DIMENSION_NAMES[key]}</span><span class='{score_class}'>{score}{score_caption}</span></div>
<div class='card-tags'>{level_tag}<span class='ctag ctag-{confidence}'>{conf_label}置信度</span><span class='ctag'>覆盖率 {coverage}%</span><span class='ctag {stage_cls}'>{html.escape(stage_label)}</span></div>
...
def generate_validation_html(interview: dict[str, Any], benchmark: dict[str, Any]) -> str:
    candidate_id = resolve_candidate_id(interview)
    return f"""<!doctype html><html lang='zh-CN'><meta charset='utf-8'><title>冰山人才分析输入校验</title><body style='font-family:Microsoft YaHei,sans-serif;padding:32px;color:#17324d'><h1>输入校验通过</h1><p>候选人编号：{html.escape(candidate_id)}</p><p>岗位：{html.escape(str(interview['position']))}</p><p>画像版本：{html.escape(str(benchmark['model_version']))}</p><p>画像状态：{html.escape(str(benchmark['benchmark_status']))}</p><p>已完成输入文件、必填字段、敏感字段键和画像治理门槛校验；未调用模型。总体评估阶段将在正式评估后根据四维证据阶段确定。</p></body></html>"""
...
def evaluate_mode(args: argparse.Namespace) -> int:
    ...
    if args.dry_run:
        target.write_text(generate_validation_html(interview, benchmark), encoding="utf-8")
