...
description: 基于冰山模型评估单个管培生或候选人的工作相关软性素质。用于在硬性条件初筛后，校验或输入脱敏简历、可选结构化面试问答和成功内化画像，输出一份包含冰山上前置事实、动机/特质/自我认知/价值观四维证据、置信度、追问和处理建议的完整 HTML 报告。支持 CLI 与 HR 本地上传体验；不用于自动录用、自动淘汰、未验证的人才预测或敏感属性推断。
---
...
- **画像确认门槛**：只有填写业务负责人、HR 负责人、审批日期、真实样本量、结果数据来源和审批说明后，画像才可标记为 `confirmed`；否则保持 `draft`。
...
在浏览器打开 `http://127.0.0.1:8765`。页面支持内置脱敏样例、三文件上传、输入校验、同页预览和 HTML 下载。正式生成前通过环境变量配置企业批准的模型服务；不要在页面或源码中填写 API Key。

## 文件说明

- `scripts/iceberg_evaluator.py`：单候选人、仅输出一份完整 HTML 报告的评估脚本。
- `scripts/hr_local_app.py`：仅绑定本机的 HR 上传、预览与下载体验服务；临时上传文件处理后立即删除。
- `assets/hr_app/index.html`：HR 本地体验页面。
